...
class MakeSfCsv:
    ...
    def make_sf_column(self,param,df,df_sf,start,end,center_atom,another_atom):
        ...
        for num_column in df.columns[start:end + 1]:
            # 4πr^2dr * sf
            df[num_column] = df[num_column] * 4 * np.pi * (float(num_column) **2) * 0.2 * \
                             acsf.radial_symmetry_function_2(eta=param[0], r_ij=float(num_column), r_shift=param[1], r_cutoff=param[2])
            df_sf[f'sf_{center_atom}-{another_atom}_e-{param[0]}_rs-{param[1]}'] = df.iloc[:, start:end + 1].sum(axis=1)
        return df_sf
...
if __name__ == '__main__':
    ...
    csv_path = '/Users/y1u0d2/Desktop/Lab/result/rdf/rdf_result/format/all/all_O-O.csv'
    obj.make_sf_csv(r_sf_O_O_params,csv_path,'sf_O-O')
    obj.plot_sf(r_sf_O_O_params,'O-O','/Users/y1u0d2/Desktop/Lab/result/sf','sf_dist_O-O')
