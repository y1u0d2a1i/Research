...
for structure in structures:
    ...
    for _dir in dirs:
        pic = glob.glob(f'{_dir}/*_min.png')[0]
        if pic:
            all_pic.append(pic)

IMG_DISPLAY_HEIGHT = Inches(3.5)  # 画像サイズ
PIC_PER_PAGE = 4
OUTPUT_FILE_NAME = '/Users/y1u0d2/desktop/Lab/result/lammps/structure-optimization/02/structure_opt_min.pptx'  # 出力ファイル名
...
for i in range(int(slides)):
    ...
    pic3 = all_pic[i*4+2]
    pic4 = all_pic[i*4+3]
    print(pic1.split('/')[-2],pic2.split('/')[-2],pic3.split('/')[-2],pic4.split('/')[-2])
